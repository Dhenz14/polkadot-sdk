...
async fn prepare_evm_transactions<Client: EthRpcClient + Sync + Send>(
	...
	let start_nonce = client
		.get_transaction_count(signer.address(), BlockNumberOrTag::Latest.into())
		.await?;
...
async fn test_transfer() -> anyhow::Result<()> {
	...
	let initial_balance =
		client.get_balance(ethan.address(), BlockNumberOrTag::Latest.into()).await?;
...
async fn test_deploy_and_call() -> anyhow::Result<()> {
	...
	let initial_balance =
		client.get_balance(contract_address, BlockNumberOrTag::Latest.into()).await?;
	assert_eq!(
...
async fn test_receipt_mixed_revert_and_logs_same_block() -> anyhow::Result<()> {
	...
	let deploy = |name, fixture_type| {
		...
		async move {
			...
			let nonce =
				client.get_transaction_count(address, BlockNumberOrTag::Latest.into()).await?;
...
async fn test_earliest_block_tag() -> anyhow::Result<()> {
	...
	// eth_feeHistory
	let fee = client
		.fee_history(U256::from(1), BlockNumberOrTag::Earliest.into(), None)
		.await?;
	...
	let traces =
		DebugRpcClient::trace_block_by_number(&*client, BlockNumberOrTag::Earliest.into(), None)
			.await?;
