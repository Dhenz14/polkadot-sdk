...
# structs, enums, and type aliases declared in this crate (after macro expansion); edges are
# containment relationships (struct fields, enum variant payloads, type-alias targets). External
# types such as `Vec`, `Option`, `H256`, `Address`, etc. are filtered out.
#
# The resulting SVG embeds a small script that, when the file is opened in any browser, lets you
# click a node to highlight every type it transitively contains and every type that transitively
# contains it.
#
# Pipeline:
#   1. `cargo +nightly rustdoc` emits a JSON dump of the crate after macro expansion, so the
#      macro-generated `Latest{Name}` aliases and per-version structs/enums show up as first-class
#      items.
...
jq -r '
	...
	# Bases of macro-generated versioned types: any name ending in `V<digits>`
	# with that suffix stripped. `BlockV1` and `BlockV2` both contribute `Block`.
	| ( $all_local
		| map(.name)
	...
	# Drop macro-generated `Latest{X}` aliases that would otherwise clutter the
	# graph as a parallel alias next to each versioned type. Two patterns are
	# excluded: aliases whose stripped name has an `XV<n>` counterpart
	# (e.g. `LatestBlock` paired with `BlockV1`), and aliases for runtime API
	# payloads whose names end in `InputPayload` or `OutputPayload`, whose
	# versioned counterparts splice `Versioned` into the middle and do not
	# match the simpler suffix rule.
	| ( $all_local
		| map(
			(.name | ltrimstr("Latest")) as $base
			| select(
				(.name | startswith("Latest") | not)
				or (
					(($versioned_bases | index($base)) == null)
					and (.name | test("(?:Input|Output)Payload$") | not)
				  )
			  )
	  )
	  ) as $nodes
	...
	| "digraph TypeGraph {",
	  ...
	  ( $nodes[]
		...
		| (
			if (.name | (
				test("^Versioned.*(?:Input|Output)Payload$")
				or test("Versioned(?:Input|Output)PayloadV[0-9]+$")
			  )) then "#cfe5ff"
			else "#ffe2c4"
			end
		  ) as $fill
...
<script type="text/ecmascript"><![CDATA[
  (function () {
    ...
    edges.forEach(function (e) {
      ...
      if (!m) return;
      var src = m[1], dst = m[2];
      (outEdges[src] = outEdges[src] || []).push({ edge: e, other: dst });
    ...
    function highlight(name) {
      ...
      // Highlight only the types that transitively depend on this one
      // (its ancestors), reached by walking incoming edges.
      walk(name, inEdges);
    }
    nodes.forEach(function (n) {
